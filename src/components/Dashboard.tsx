...
const Dashboard: React.FC<DashboardProps> = ({ userProfile, hasCompletedAssessment }) => {
  const stats = [
    { label: 'Career Matches', value: hasCompletedAssessment ? '12' : '0', icon: Target, color: 'bg-blue-500' },
    { label: 'Skills Assessed', value: userProfile.skills.technical.length + userProfile.skills.soft.length, icon: Brain, color: 'bg-green-500' },
    { label: 'Learning Paths', value: hasCompletedAssessment ? '8' : '0', icon: BookOpen, color: 'bg-purple-500' },
    { label: 'Progress Score', value: hasCompletedAssessment ? '78%' : '15%', icon: Award, color: 'bg-orange-500' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-8 text-white">
        ...
        {!hasCompletedAssessment && (
          <div className="mt-6">
            <button className="bg-white text-blue-600 px-6 py-3 rounded-xl font-semibold hover:bg-gray-50 transition-colors flex items-center space-x-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => {
          ...
          return (
            <div key={index} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-200">
              <div className="flex items-center justify-between">
                <div>
              ...
              return (
                <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-900">{trend.industry}</p>
                      <p className="text-sm text-gray-600">{trend.jobs} open positions</p>
                    </div>
                  </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button className="p-4 border-2 border-dashed border-gray-300 rounded-xl hover:border-blue-500 hover:bg-blue-50 transition-all duration-200 group">
            <Target className="w-8 h-8 text-gray-400 group-hover:text-blue-600 mx-auto mb-2" />
            <p className="font-medium text-gray-700 group-hover:text-blue-600">Explore Careers</p>
          </button>
          <button className="p-4 border-2 border-dashed border-gray-300 rounded-xl hover:border-green-500 hover:bg-green-50 transition-all duration-200 group">
            <BookOpen className="w-8 h-8 text-gray-400 group-hover:text-green-600 mx-auto mb-2" />
            <p className="font-medium text-gray-700 group-hover:text-green-600">Start Learning</p>
          </button>
          <button className="p-4 border-2 border-dashed border-gray-300 rounded-xl hover:border-purple-500 hover:bg-purple-50 transition-all duration-200 group">
            ...
            <p className="font-medium text-gray-700 group-hover:text-purple-600">Connect</p>
          </button>
        </div>
      </div>
    </div>
